...
class BNSIPCTranslatorService {
    ...
    static async analyzeComplaintWithML(complaint, language = "hi") {
        ...
        try {
            ...
            const sectionNumbers = mlResponse.classifications
                ?.map((c) => c.section_number ?? c.sectionNumber)
                .filter((v) => typeof v === "string" && v.trim().length > 0) || [];
            console.log(`[BNS ML] Extracted section numbers:`, sectionNumbers);
            // Fetch full section details from database
            const sections = sectionNumbers.length > 0
                ? await database_1.prisma.bNSSection.findMany({
                    where: {
                        sectionNumber: { in: sectionNumbers },
                    },
                })
                : [];
            console.log(`[BNS ML] Found ${sections.length} matching sections in database`);
            return {
                sections,
                mlResponse,
            ...
            throw new ApiError_1.ApiError(500, `Failed to analyze complaint: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
}
