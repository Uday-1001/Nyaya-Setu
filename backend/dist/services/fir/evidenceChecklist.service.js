...
class EvidenceChecklistService {
    ...
    static async generateChecklistForFIR(firId) {
        ...
        if (fir.bnsSections.length === 0) {
            throw new ApiError_1.ApiError(400, 'FIR must have at least one BNS section to generate checklist');
        }
        // Get checklist template for primary BNS section
        const primarySection = fir.bnsSections[0];
        const template = await database_1.prisma.evidenceChecklist.findFirst({
     ...
     */
    static async createDefaultChecklist(bnsSectionId) {
        const bnsSection = await database_1.prisma.bNSSection.findUnique({
        ...
        const checklist = await database_1.prisma.evidenceChecklist.create({
            data: {
                bnsSectionId,
     ...
     */
    static getDefaultChecklistItems(category) {
        const templates = {
            VIOLENT: [
                {
     ...
     */
    static async getChecklistForFIR(firId) {
        const fir = await database_1.prisma.fIR.findUnique({
            where: { id: firId },
            include: { bnsSections: true },
        });
        ...
        const primarySection = fir.bnsSections[0];
        if (!primarySection) {
            throw new ApiError_1.ApiError(400, 'FIR has no BNS section');
        }
        const checklist = await database_1.prisma.evidenceChecklist.findFirst({
     ...
     */
    static async getEvidenceStatus(firId) {
        const evidenceItems = await database_1.prisma.evidenceItem.findMany({
            where: { firId },
     ...
     */
    static async markEvidenceCollected(evidenceItemId, fileUrl) {
        const item = await database_1.prisma.evidenceItem.findUnique({
            where: { id: evidenceItemId },
        });
        if (!item) {
            throw new ApiError_1.ApiError(404, 'Evidence item not found');
        }
        return database_1.prisma.evidenceItem.update({
            where: { id: evidenceItemId },
            data: {
                isCollected: true,
     ...
     */
    static async addEvidenceItem(firId, checklistItemId, label, notes) {
        const fir = await database_1.prisma.fIR.findUnique({
            where: { id: firId },
        });
        if (!fir) {
        ...
        return database_1.prisma.evidenceItem.create({
            data: {
                firId,
                checklistItemId,
                label,
...
exports.EvidenceChecklistService = EvidenceChecklistService;
