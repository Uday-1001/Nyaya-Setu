...
class AnomalyDetectionService {
     ...
     */
    static async analyzeFIRForAnomalies(firId) {
        ...
        const recentFIRs = await database_1.prisma.fIR.findMany({
            where: {
        ...
        if (recentFIRs.length > 3) {
            flags.push(`Complainant has filed ${recentFIRs.length} FIRs in the last 30 days (typically not normal)`);
            riskScore += 20;
        }
        ...
        if (!fir.incidentLocation)
            missingDetails.push('location');
        if (fir.victimStatements.length === 0)
            missingDetails.push('victim statement');
        ...
        const historicalFIRs = await database_1.prisma.caseUpdate.findMany({
            where: {
                fir: {
                    victimId: fir.victimId,
                },
                status: enums_1.FIRStatus.REJECTED,
            },
        });
        ...
        const similarIncidents = await database_1.prisma.fIR.findMany({
            where: {
                victimId: fir.victimId,
     ...
     */
    static analyzeStatementForContradictions(statementText, otherStatements) {
     ...
     */
    static async generateInquiryRecommendations(firId) {
        const fir = await database_1.prisma.fIR.findUnique({
            where: { id: firId },
            include: {
                victim: true,
        ...
        const daysSinceFiling = Math.floor((new Date().getTime() - new Date(fir.createdAt).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSinceFiling > 7 && fir.status === enums_1.FIRStatus.DRAFT) {
    ...
    static async checkInvestigationDelays() {
        ...
        const delayedFIRs = await database_1.prisma.fIR.findMany({
            where: {
                createdAt: {
                    lte: nineDaysAgo,
                },
                status: {
                    in: [enums_1.FIRStatus.ACKNOWLEDGED, enums_1.FIRStatus.UNDER_INVESTIGATION],
                },
            },
            include: {
                officer: true,
        ...
        return delayedFIRs.map((fir) => ({
            ...
            daysInvestigating: Math.floor((new Date().getTime() - new Date(fir.createdAt).getTime()) / (1000 * 60 * 60 * 24)),
            status: fir.status,
