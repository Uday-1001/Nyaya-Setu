...
exports.normalizeClassifyOnlyResponse = exports.fullPipelineToClassificationPayload = exports.normalizeFullPipelineResponse = void 0;
...
const parseUrgency = (v) => {
    const s = String(v ?? '')
        .trim()
        .toUpperCase();
    if (s === 'CRITICAL')
        return enums_1.UrgencyLevel.CRITICAL;
    if (s === 'HIGH')
        return enums_1.UrgencyLevel.HIGH;
    if (s === 'LOW')
        return enums_1.UrgencyLevel.LOW;
    return enums_1.UrgencyLevel.MEDIUM;
};
const parseClassifications = (raw) => {
    if (!Array.isArray(raw))
        return [];
    const out = [];
    for (const item of raw) {
        const o = asRecord(item);
        const sectionNumber = asString(o.section_number) ??
...
const normalizeFullPipelineResponse = (json) => {
    const o = asRecord(json);
    const transcript = asString(o.transcript) ?? asString(o.whisper_text) ?? asString(o.asr_text) ?? '';
    const rawComplaintText = asString(o.raw_complaint_text) ??
    ...
    const primaryFromField = asString(o.primary_section_number) ??
        ...
        classifications = [
            {
                ...
                confidence: asNumber(o.primary_confidence) ?? asNumber(o.primaryConfidence) ?? asNumber(o.confidence) ?? 0.72,
            },
        ];
    }
    if (!primarySection || classifications.length === 0) {
    ...
    const victimRightsBullets = Array.isArray(bulletsRaw)
        ? bulletsRaw.filter((x) => typeof x === 'string')
        : undefined;
    return {
        transcript,
        rawComplaintText: rawComplaintText.trim() || transcript,
        ...
        urgencyReason: asString(o.urgency_reason) ??
            asString(o.urgencyReason) ??
            'Review suggested based on automated analysis.',
        severityScore: asNumber(o.severity_score) ??
            asNumber(o.severityScore) ??
            classifications[0]?.confidence ??
            0.5,
        victimRightsSummary: asString(victimRights.summary) ?? asString(victimRights.text) ?? asString(o.rights_summary),
        victimRightsBullets,
        modelVersion: asString(o.model_version) ?? asString(o.modelVersion) ?? 'ml-unknown',
    };
};
exports.normalizeFullPipelineResponse = normalizeFullPipelineResponse;
const fullPipelineToClassificationPayload = (p) => {
...
exports.fullPipelineToClassificationPayload = fullPipelineToClassificationPayload;
/** Classifier-only JSON (e.g. `/v1/classify` — IndicBERT multi-label without ASR). */
const normalizeClassifyOnlyResponse = (json) => {
    const o = asRecord(json);
    let classifications = parseClassifications(o.classifications ?? o.bns_sections ?? o.labels);
    const primaryFromField = asString(o.primary_section_number) ??
        asString(o.primarySectionNumber) ??
...
exports.normalizeClassifyOnlyResponse = normalizeClassifyOnlyResponse;
