...
const wrapText = (value: string, width = 92) => {
...
export class FIRPdfService {
  static async generateForFir(firId: string): Promise<PDFBuildResult> {
    ...
    const sections = fir.bnsSections.length
      ? fir.bnsSections.map((section) => ({
          ...
          reasoning: (section as typeof section & { mappingReasoning?: string | null }).mappingReasoning ?? null,
        }))
    ...
    const lines = [
      ...
      ...(fir.caseUpdates.length
        ? fir.caseUpdates.flatMap((entry) =>
            wrapText(`- ${entry.createdAt.toISOString()} / ${entry.status} / ${entry.note ?? 'No note'}`, 92),
          )
