...
import bnsDatabase from "../../data/bns_map.json";
...
type BNSSection = (typeof bnsDatabase)[0];

const getSectionGist = (section: BNSSection) => {
  ...
  const hasPlaceholderDescription =
    /auto-mapped from huggingface dataset\.?/i.test(description);
  ...
  const titleForSentence = normalizedTitle
    ? normalizedTitle.charAt(0).toLowerCase() + normalizedTitle.slice(1)
    : "this offense";

  if (!description || hasPlaceholderDescription) {
    return `This section explains ${titleForSentence}.`;
  }

  if (/short title, commencement and application/i.test(description)) {
    return "This section explains the name of the law part, when it starts, and where it applies.";
  }
...
export const AdminBNSCatalogue = () => {
  const [query, setQuery] = useState("");

  const filteredData = useMemo(() => {
    if (!query.trim()) return bnsDatabase;
    const q = query.toLowerCase();
    return bnsDatabase.filter(
      (sec) =>
        sec.sectionNumber.toLowerCase().includes(q) ||
        sec.sectionTitle.toLowerCase().includes(q) ||
        sec.ipcEquivalent.toLowerCase().includes(q) ||
    ...
    >
      ...
      >
        <div>
          ...
          <h1 className="text-3xl lg:text-4xl font-extrabold tracking-tight text-white mb-3 flex items-center gap-4">
            ...
            Global BNS Catalogue
          </h1>
          <p className="max-w-2xl text-sm text-[#D1D5DB] leading-relaxed">
            Live index of the {bnsDatabase.length} comprehensive BNS sections
            mapped natively into the HuggingFace AI classification model.
          </p>
        </div>
      ...
      <motion.div variants={itemVariants} className="mb-8 relative max-w-xl">
        ...
        <input
          ...
          placeholder="Search by BNS Number, Title, or IPC equivalent..."
...
export default AdminBNSCatalogue;
